...
const handleDeleteTicket = (id) => {
  const user = authService.getCurrentUser();
  api.delete(`api/tickets/${id}`, {   params:{'cargo': user.cargo},     headers: authHeader()})
  window.location.reload()
}
...
const handleGetTicket = () => {
    ...
    api.get(`api/clients/getuserclient/${user.user}`, {params:{'cargo': user.cargo}, headers: authHeader()})
    .then((data) => {
    setClient(data.data);
    console.log(clients)
